...
// Fabien
        ...
        {
            //$dateJour = date('Y-m-d');
            $req = "SELECT idVisiteur, mois, dateModif, idEtat, montantValide FROM fichefrais WHERE mois > 201612 AND idVisiteur = :id_Vis AND (idEtat = 'RB' OR idEtat = 'VA')";
            $Aff_Info = DB::select($req, ['id_Vis'=>$id]);
            return $Aff_Info;
        }
        
        public function Aff_Detail_HorsForfait_Comptable($idVisiteur,$mois){
	    $req = "select * from lignefraishorsforfait where lignefraishorsforfait.idvisiteur =:idVisiteur 
		and lignefraishorsforfait.mois = :mois";	
            $FHF = DB::select($req, ['idVisiteur'=>$idVisiteur, 'mois'=>$mois]);
            return $FHF; 
	}
        
        public function Aff_Detail_HorsForfait($idVisiteur,$mois){
	    $req = "select * from lignefraishorsforfait where lignefraishorsforfait.idvisiteur =:idVisiteur 
		and lignefraishorsforfait.mois = :mois AND lignefraishorsforfait.Supprimé = 0";	
            $FHF = DB::select($req, ['idVisiteur'=>$idVisiteur, 'mois'=>$mois]);
            return $FHF; 
	}
        
        public function Aff_Detail_HorsForfaitSup($idVisiteur,$mois){
	    $req = "select * from lignefraishorsforfait where lignefraishorsforfait.idvisiteur =:idVisiteur 
		and lignefraishorsforfait.mois = :mois AND lignefraishorsforfait.Supprimé = 1";
            $FHF = DB::select($req, ['idVisiteur'=>$idVisiteur, 'mois'=>$mois]);
            return $FHF; 
	}
        
        public function Aff_DetailForfait($idVisiteur, $mois){
		$req = "select fraisforfait.id as idfrais, fraisforfait.libelle as libelle, ligneFraisForfait.mois as mois,
		lignefraisforfait.quantite as quantite from lignefraisforfait inner join fraisforfait 
		on fraisforfait.id = lignefraisforfait.idfraisforfait
		where lignefraisforfait.idvisiteur = :idVisiteur and lignefraisforfait.mois=:mois
		order by lignefraisforfait.idfraisforfait";
                $FF = DB::select($req, ['idVisiteur'=>$idVisiteur, 'mois'=>$mois]);
		return $FF;
	}
